...
取得 EFI_FILE_PROTOCOL* Root（Root 目錄）

Root->Open() 開檔（Read / Write / Create）

File->Read() / File->Write() / File->Delete() / File->GetInfo()

File->Close()（Delete 例外：Delete 會自動 Close）
...
先用 LoadedImage->DeviceHandle 嘗試拿 SimpleFS（最符合“目前裝置”）

若失敗就 fallback：掃描第一個 SimpleFS
...
File->GetInfo() 拿 FileSize

AllocatePool(FileSize+1)
